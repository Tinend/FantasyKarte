...
seitenLaenge.times do |y|
  seitenLaenge.times do |x|
    grau = [[(wasser.berechneSichtbarkeit([(x - seitenLaenge / 2) / schrittWeite, (y - seitenLaenge / 2) / schrittWeite,1]) * 256 - 0.5).round, 0].max, 255].min
    png[x, y] = ChunkyPNG::Color.rgb(grau, grau, grau)
  end
  puts "#{y + 1} von #{seitenLaenge}"
end
png.save('../ausgabe/test2Wasser.png', :interlace => true)
